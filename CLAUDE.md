...
pnpm, not npm. Node ≥ 22.12. `astro check` needs TypeScript 6.x — 7.x drops the
API it depends on, so **do not bump TypeScript to 7**.
...
**`pnpm build` and `pnpm check` must both pass before committing.** That's the
only gate between a commit and production.
