...
| renders once and never changes                       | `.astro` — adds zero JS                    |
...
time. It adds no JavaScript of its own, renders instantly, prints correctly,
and works with JS disabled. Reach for a React island only when the reader
actually manipulates something — dragging, filtering, stepping through state.
